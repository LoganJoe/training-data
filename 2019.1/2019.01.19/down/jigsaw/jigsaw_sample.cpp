...
Answer solve(Image img, int H, int W, int h, int w)
{
    int h_cnt = H / h, w_cnt = W / w;
    Answer res;
    res = Answer(h_cnt, Answer_Row(w_cnt));
    for (int hi = 0; hi < h_cnt; ++ hi)
        for (int wi = 0; wi < w_cnt; ++ wi)
            res[hi][wi] = make_pair(hi, wi);
...
int main(int argc, char *argv[])
{
    ...
    res = solve(input, H, W, h, w);

    ofstream out("jigsaw.out", ios::app);
    out << file_name << endl;
    out << h_cnt - 2 << " " << w_cnt - 2 << endl;
